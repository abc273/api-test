...
- 文档适用对象：调用 8liangAI 图片生成、视频生成、任务查询、视频查看、真实资产查询、虚拟资产查询接口的客户开发人员
- 文档编写原则：站在客户接入视角说明“怎么调、每个参数怎么传、什么时候用、返回什么、常见坑是什么”
- 当前确认可用的 4 个模型别名：
  - 图片模型：`seedream4.5`、`seedream5.0lite`
  - 视频模型：`seedance2`、`sd2.0fast`
...
本项目不是所有接口都返回同一种 JSON 结构，请客户特别注意：
...
当前 `seedance2` 和 `sd2.0fast` 已接入“可选付费”的视频超分能力。

对客户侧的实际表现是：

1. 默认不启用视频超分，不会自动产生这部分额外费用
2. 只有当您在请求里显式传 `metadata.enable_video_super_resolution = true` 时，系统才会继续执行视频超分
3. 如果您提交的是 `480p` 视频，启用超分后，最终返回的视频会提升到 `720p`
4. 如果您提交的是 `720p` 视频，启用超分后，最终返回的视频会提升到 `1080p`
5. 如果您提交的已经是 `1080p`，系统不会再继续超分
6. 开启超分后，任务只有在超分处理完成后，才会对外显示为最终成功

这意味着：

- 未开启超分时，`GET /v1/video/generations/{task_id}` 和 `GET /v1/videos/{task_id}/content` 返回的是原始生成视频
- 开启超分后，`GET /v1/video/generations/{task_id}` 返回的最终 `result_url`，以及 `GET /v1/videos/{task_id}/content` 取到的，默认都是超分后的成品视频
- 相比不做超分的普通视频任务，开启超分后的整体完成时间可能会略长，这是正常现象

建议：

- `seedance2` 和 `sd2.0fast` 都显式传 `metadata.resolution`
- 如果您希望最终拿到 `1080p`，最稳妥的传法是直接提交 `720p`，并显式打开 `metadata.enable_video_super_resolution`
- `sd2.0fast` 仍然不要传 `1080p`

### 3.3 真实资产/虚拟资产使用

当前客户侧文档只开放“查看真实资产、查看虚拟资产、拿到资产 ID、在视频生成中引用资产”这部分能力。
...
下表列出当前 8liangAI 网关对外接受的图片字段。右侧建议已经按官方《图片生成 API》和当前这 2 个图片模型的实际能力收敛，客户可直接按此传参，不需要再自己去核对上游文档。
...
| `style` | string / object / array | 否 | JSON body | `"photorealistic"` | 风格参数，原样透传 | 当前这 2 个模型的客户公开文档先不作为标准参数，不建议对客户承诺 |
...
| `background` | string / object | 否 | JSON body | `"transparent"` | 背景设置 | 当前这 2 个模型的客户公开文档先不作为标准参数，不建议传 |
| `moderation` | string / object | 否 | JSON body | `"low"` | 安全审核相关参数 | 当前这 2 个模型的客户公开文档先不作为标准参数，不建议传 |
| `output_format` | string / object | 否 | JSON body | `"png"` | 输出格式 | `seedream5.0lite` 支持 `png`/`jpeg`，默认 `jpeg`；`seedream4.5` 固定 `jpeg`，不要传这个字段 |
| `output_compression` | number / string | 否 | JSON body | `80` | 输出压缩率 | 当前这 2 个模型的客户公开文档先不作为标准参数，不建议传 |
| `partial_images` | number / object | 否 | JSON body | `1` | 中间结果相关配置 | 当前这 2 个模型的客户公开文档先不作为标准参数，不建议传 |
...
4. 当前公开客户文档不建议把 `style`、`background`、`moderation`、`output_compression`、`partial_images` 当成标准交付参数对客户承诺；这些字段虽可被网关接收，但不属于这 2 个模型的主推荐接入路径
...
当前 `seedance2` / `sd2.0fast` 能接收并转上游的视频扩展参数如下：

| 参数名 | 类型 | 必填 | 传法示例 | 含义 | 建议 |
| --- | --- | --- | --- | --- | --- |
| `resolution` | string | 强烈建议 | `"720p"`、`"1080p"` | 输出分辨率 | 对当前客户接入来说请显式传。`seedance2` 在不传分辨率时，实际联调中很容易直接触发价格档位匹配失败；`seedance2` 支持 `480p`/`720p`/`1080p`；`sd2.0fast` 只建议用 `480p`/`720p`，不支持 `1080p` |
...
| `enable_video_super_resolution` | boolean | 否 | `true` | 是否启用可选付费视频超分 | 默认不启用。只有显式传 `true` 时，系统才会在原始视频生成完成后继续做超分 |
| `tools` | array | 否 | `[{"type":"web_search"}]` | 工具扩展 | 当前四模型通常不需要 |
| `content` | array | 否 | 见后文 | 高级多模态内容数组 | 用来直通官方 `content[]` 结构；普通客户不建议手写，确需传音频/视频/首尾帧时再用 |
...
- 普通客户：只传顶层 `duration`
- 高级客户：如需跟上游参数结构保持一致，可在 `metadata` 中同步保留，但以顶层为准

补充说明：

- 火山方舟官方对 Seedance 2.0 系列支持 `duration=-1` 的“智能时长”模式
- 但在当前 8liangAI 客户接入里，顶层 `duration` 最稳妥的用法仍是传 `4-15` 的正整数
...
6. 视频超分是可选付费能力，默认不启用；只有显式传 `metadata.enable_video_super_resolution=true` 时才会触发
7. 如果您开启超分并提交 `480p`，最终通常会拿到 `720p`；如果您开启超分并提交 `720p`，最终通常会拿到 `1080p`
6. 真实资产和虚拟资产接入成功后，视频接口可以直接复用资产，减少重复上传
...
- 当前对客户仅开放“查看真实资产列表、查看真实资产 ID、查看预览”
- 创建、上传、同步、确认、回调等接口暂不在客户文档中披露
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | int | 真实资产任务 ID。客户如需和平台沟通某条资产记录，通常报这个值 |
| `user_id` | int | 用户 ID |
| `name` | string | 任务/资产名称 |
...
| `asset_id` | string | 真实资产 ID。客户最需要关注的字段之一，后续视频生成引用资产时可用 |
...
如果客户当前只需要“查看真实资产并拿到资产 ID”，建议重点关注以下字段：
...
这个接口里的 `{state}` 不是让客户自己拼的，而是平台在列表接口返回的 `asset_preview` 完整 URL 中已经带好了。
...
- 当前对客户仅开放“查看虚拟资产列表、查看虚拟资产 ID、查看预览”
- 创建、上传、同步、分组等接口暂不在客户文档中披露
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | int | 虚拟资产记录 ID。客户如需和平台沟通某条资产记录，通常报这个值 |
| `user_id` | int | 用户 ID |
| `group_id` | int | 分组 ID |
| `name` | string | 资产名称 |
| `asset_type` | string | `Image` / `Video` / `Audio` |
...
| `volc_asset_id` | string | 虚拟资产 ID。客户最需要关注的字段之一，后续视频生成引用资产时可用 |
| `status` | string | 当前业务状态 |
...
如果客户当前只需要“查看虚拟资产并拿到资产 ID”，建议重点关注以下字段：
...
如果客户的核心业务是“先创建资产，再生成视频”，推荐按以下方式对接：
...
5. 如果模型是 `seedance2` 或 `sd2.0fast`，并且您显式开启了 `metadata.enable_video_super_resolution=true`，任务在原始视频生成完成后还可能继续进入超分阶段，因此整体耗时会比普通任务更长
