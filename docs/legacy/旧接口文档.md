...
本文档是 8liangai.com 当前线上接口的标准接入文档，覆盖对话模型、视频生成、图片生成、真人资产、虚拟人像资产、API Key 创建和资产创建教程。
...
- 对话模型统一使用 `DeepSeek`
- 如果客户端不需要展示推理过程，请忽略返回中的 `reasoning_content`
...
对话、视频、图片等 OpenAI 兼容接口发生错误时通常返回：
...
| `user` | `string` | 否 | `user-123` | 业务侧自定义用户标识 |
...
| 设定助手风格 | 第一条传 `system` |
| 用户提问 | 传 `user` |
| 带历史上下文 | 按顺序传入历史 `assistant` 与 `user` 消息 |
| 单轮问答 | 只传一条 `user` 消息即可 |
...
| `choices[].message.role` | `string` | 一般为 `assistant` |
| `choices[].message.content` | `string` | 最终正文内容 |
| `choices[].message.reasoning_content` | `string` | 推理内容，如不展示可忽略 |
| `usage.prompt_tokens` | `integer` | 输入 token 数 |
| `usage.completion_tokens` | `integer` | 输出 token 数 |
| `usage.total_tokens` | `integer` | 总 token 数 |
| `usage.prompt_tokens_details.cached_tokens` | `integer` | 缓存命中 token 数 |
...
| --- | --- |
| `data:` | 每个 SSE 数据块前缀 |
| `choices[].delta.content` | 正文增量内容，应拼接该字段 |
| `choices[].delta.reasoning_content` | 推理增量内容，可按需展示或忽略 |
| `choices[].finish_reason` | 结束原因 |
| `usage` | 通常出现在最后一个有效 chunk 中 |
| `[DONE]` | 流结束标记 |
...
| `callback_url` | `string` | 否 | 必须为 `http://` 或 `https://` URL | 真人实名校验完成后，八两 API 处理完认证结果后跳转到该地址 |
...
{
  "name": "主播真人资产",
  "callback_url": "https://your-domain.com/portrait/callback"
}
```
...
- 火山实名校验的上游回调仍会先进入八两 API，用于写入认证结果和素材组。
- 如果创建任务时传入 `callback_url`，八两 API 处理完成后会跳转到该地址。
- 跳转时会追加 `job_id` 和 `result_code` 查询参数，例如：

```text
https://your-domain.com/portrait/callback?job_id=12&result_code=10000
```

- 如果未传 `callback_url`，认证完成后仍跳转到八两 API 的真人资产页面。
...
| --- | --- | --- |
| 模型列表 | `GET` | `/v1/models` |
| 单模型详情 | `GET` | `/v1/models/{model}` |
| 对话补全 | `POST` | `/v1/chat/completions` |
| 视频创建 | `POST` | `/v1/videos` |
| 视频状态查询 | `GET` | `/v1/videos/{task_id}` |
| 视频内容下载 | `GET` | `/v1/videos/{task_id}/content` |
| 图片生成 | `POST` | `/v1/images/generations` |
...
### 13.4 DeepSeek 对话上线前检查

| 检查项     | 要求                                   |
| ------- | ------------------------------------ |
| 推荐模型名   | 当前使用 `DeepSeek`                      |
| 单模型详情接口 | 不要依赖 `/v1/models/{model}` 做强校验       |
| 推理内容    | 明确是否向最终用户展示 `reasoning_content`      |
| 流式处理    | 只拼接 `delta.content`                  |
